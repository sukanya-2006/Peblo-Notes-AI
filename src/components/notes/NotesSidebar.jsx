...
export default function NotesSidebar({
  ...
  return (
    <aside className="w-[290px] h-full flex flex-col sidebar-surface shrink-0 relative overflow-hidden backdrop-blur-2xl">

      {/* Ambient glow */}
      <div className="absolute top-0 left-0 w-56 h-56 bg-violet-600/10 rounded-full blur-3xl pointer-events-none -translate-x-1/2 -translate-y-1/2" />

      {/* Header */}
      <div className="relative flex items-center gap-3 px-5 pt-6 pb-5">

        <div className="float-soft w-9 h-9 rounded-2xl bg-gradient-to-br from-violet-500 via-violet-600 to-indigo-500 flex items-center justify-center shadow-[0_0_30px_rgba(139,92,246,0.35)] shrink-0">
          <StickyNote size={15} className="text-white" />
        </div>

        <div>
          <span className="text-white text-[15px] font-bold tracking-[-0.03em] leading-none gradient-text">
            Peblo Notes
          </span>

          <p className="text-white/30 text-[10px] mt-1 leading-none tracking-wide">
            Your second brain
          </p>
        </div>
      </div>
      ...
      <div className="px-3 pb-4">
        <div className="glass-panel flex gap-1 p-1 rounded-2xl">
      ...
      <div className="px-3 pb-4">
        ...
        <div
          className={`glass-panel flex items-center gap-2.5 px-3 py-2.5 rounded-2xl transition-all duration-300 ${
            search
              ? "border-violet-500/30 shadow-[0_0_25px_rgba(139,92,246,0.15)]"
              : ""
          }`}
        >
      ...
      <AnimatePresence>
        {allTags.length > 0 && (
          ...
          >
            ...
            <p className="text-white/20 text-[10px] uppercase tracking-[0.22em] font-medium mb-2 px-1">
              Tags
            </p>
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2 scrollbar-none">
        ...
        {notes.length === 0 ? (
          ...
          >
            ...
            <div className="glass-panel w-12 h-12 rounded-2xl flex items-center justify-center">
              <FileText
                size={18}
            ...
            {notes.map((note, i) => {
              ...
              return (
                ...
                <motion.button
                  ...
                  className={`note-card glass-panel w-full text-left px-3.5 py-3.5 rounded-2xl transition-all duration-300 group relative overflow-hidden ${
                    isActive
                      ? "bg-violet-500/10 border border-violet-400/20 shadow-[0_0_30px_rgba(139,92,246,0.12)]"
                      : "hover:bg-white/[0.04]"
                  }`}
                >
                  ...
                  {isActive && (
                    <motion.div
                      layoutId="activeBar"
                      className="absolute left-0 top-3 bottom-3 w-[3px] bg-gradient-to-b from-violet-300 to-violet-500 shadow-[0_0_20px_rgba(139,92,246,0.6)] rounded-full"
                    />
                  )}
                  ...
                  <div className="flex items-start justify-between gap-2 pl-1">
                    ...
                    <p
                      className={`sidebar-note-title text-[13.5px] leading-snug truncate flex-1 transition-colors ${
                        isActive
                          ? "text-white"
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      {note.aiUsed && (
                        <span className="text-[9px] text-violet-300 bg-violet-500/10 px-1.5 py-0.5 rounded-md border border-violet-500/20">
                          AI
                        </span>
                      )}
                    </div>
                  ...
                  <p className="text-white/35 text-[12px] font-medium mt-1.5 truncate pl-1 leading-snug">
                    {truncate(
                      note.content,
                      50
                    ) || "Empty note"}
                    ...
                    <div className="flex gap-1 flex-wrap">
                      ...
                      {(note.tags || [])
                        ...
                        .map((tag) => (
                          ...
                          <span
                            key={tag}
                            className="text-[10px] text-violet-300/70 bg-violet-500/[0.08] px-1.5 py-0.5 rounded-md"
                          >
                            #{tag}
                          </span>
                        ))}
      ...
      <div className="p-3 border-t border-white/[0.05]">
        ...
        <motion.button
          ...
          onClick={onCreateNote}
          className="smooth-button w-full flex items-center justify-center gap-2 py-3 rounded-2xl text-[13px] font-semibold text-white transition-all duration-200 relative overflow-hidden group bg-gradient-to-r from-violet-500 via-violet-600 to-indigo-500 shadow-[0_0_35px_rgba(139,92,246,0.35)]"
        >

          <span className="absolute inset-0 bg-white/0 group-hover:bg-white/[0.06] transition-colors duration-200 rounded-2xl" />
...
function NavBtn({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className={`relative flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-[12px] font-semibold transition-all duration-200 ${
        active
          ? "text-white"
          : "text-white/30 hover:text-white/60"
      }`}
    >
      ...
      {active && (
        <motion.div
          layoutId="navPill"
          className="absolute inset-0 bg-violet-500/20 border border-violet-500/25 rounded-xl"
        />
      )}
